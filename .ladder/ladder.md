...
- [ ] **R103** — Recovering the remaining 4 matches: by-ID fixture lookup bypasses the free-tier date block → *small*
  - Context: confirmed directly against the live API-Football that the 4 unrecoverable matches from R102's backfill (Chelsea v Brighton 1557379, Leeds v Brentford 1557382, Sunderland v Fulham 1557385, Man Utd v Ipswich 1557384) ARE fully fetchable (fixture metadata + 2 confirmed lineups + odds each) when queried by fixture ID directly (`/fixtures?id=X`), even though the date-bucket query (`fixtures_by_date`) that `backfill_missed_fixtures.py` currently uses is blocked by the free tier for dates that old
  - Why: this means all 4 remaining matches are genuinely recoverable, not a hard data-availability wall as R102 concluded - the limitation was specific to the backfill script's date-based fetch method, not the API's actual data availability
  - [ ] Go through the same brainstorm -> spec -> plan -> subagent-driven-development process used for every other change this session
  - [ ] Implement directly without the full process, given how small/obvious the fix is (extend the backfill script to accept explicit fixture IDs, fetching each by ID instead of relying on fixtures_by_date)
  - Blocked by: waiting on user's choice of process
