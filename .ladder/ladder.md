...
- [ ] **R046** — Vercel project has no rootDirectory set: fix now or leave as a known workaround? → *small*
  ...
  - [x] Set `rootDirectory=dashboard` in the Vercel project settings — tried via `vercel api -X PATCH .../projects/{id}` (CLI has no direct settings command, but `vercel api` gives authenticated REST access). **Wrong diagnosis, reverted.** This project's normal deploy path uploads the local `dashboard/` directory content directly as the deployment root (confirmed: plain `vercel deploy --prod --yes` run from inside `dashboard/` uploads, doesn't clone, when rootDirectory is unset) — setting rootDirectory="dashboard" broke that path immediately (`The specified Root Directory "dashboard" does not exist`, since the uploaded tarball root already IS dashboard's contents, so it looked for dashboard/dashboard). The original failed deploy earlier in this session that showed "Cloning github.com/..." was a one-off: CLI appears to sometimes prefer referencing a matching pushed git commit over re-uploading identical local bytes, and that specific path needs rootDirectory=dashboard while the normal upload path needs it unset - the two invocation styles want opposite settings, so no single static value satisfies both.
  - [ ] Leave as-is, keep using `--archive=tgz` on every future manual deploy — no settings change; forces the upload path unconditionally, sidestepping the git-clone-sometimes issue entirely without breaking the normal case. Confirmed working after revert.
  - Blocked by: ~none~
  - Status: done — reverted rootDirectory to null/unset (confirmed via `vercel api` GET), redeployed successfully with `--archive=tgz`. Conclusion: keep using `--archive=tgz` for all future manual deploys of this project; don't set rootDirectory.
