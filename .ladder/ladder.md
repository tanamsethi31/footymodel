...
- [ ] **R004** — 3-season WhoScored scrape: PL + Bundesliga (2023/24 done, 2024/25 + 2025/26 needed) → *large*
  ...
  - Blocked by: ~none~
  - Note: Both 2025/26 legs done via the same iframe-based scrape harness (extractMatch reads tbody rows off the livestatistics page's two summary tables, player_id from the /players/ link, shots/ShotsOT columns by header index, sub-minute from the 3rd player-meta-data span) driven from a single foreground tab so background-tab JS throttling never triggers. Match list source differs by league: PL used DOM link-scraping off the fixtures page; Bundesliga used the JSON fixtures API directly (`https://www.whoscored.com/tournaments/{stageId}/data/?d=YYYYMM&isAggregate=false`, one call per month, no UI clicking needed) — much faster, and confirmed the URL slug is cosmetic-only (any placeholder works, only the match ID routes). Batches of 8-10 downloaded as a Blob to ~/Downloads then merged+deduped into the TSV by match_id. Recurring "CDP Runtime.evaluate timed out" on the await call is cosmetic — the page-side loop keeps running; poll `Object.keys(window.__results).length` and/or take a `computer` screenshot to unstick the connection, don't re-run the batch. Two real interruptions handled: an ad hijacked the foreground tab mid-batch (recovered by recomputing remaining match_ids from the on-disk TSV, the source of truth, and reinstalling the harness fresh), and a Chrome extension disconnect (harness state survived in `window`, just re-poll after it reconnects). Next up: PL 2024/25, then Bundesliga 2024/25 — build each league's match_list via the JSON fixtures API (faster than DOM scraping) before running the harness.
  - Status: in_progress
