...
- [ ] **R104** — "Ideal scenario" discussion: automatic scheduling + richer phone notifications → *medium*
  - Context: user described their ideal end state - automatic routine runs on set intervals or PL-calendar-driven timing (20min before each match), plus a phone notification explaining "how we got to those analysis." Investigated first: `live_poll.yml` already runs automatically every 20min across the whole matchday window (9-21 UTC, now resilient to cron gaps per R102/R103), and the bell icon (just redesigned) already delivers real push notifications via `scripts/notify_dashboard.py` - so most of the "ideal scenario" is already live, not a gap
...
- [x] **R105** — Richer notification content: plan execution mode → *small*
...
- [x] **R105** — Richer push notification content: shipped end-to-end → *medium*
  - Context: subagent-driven execution of both tasks from docs/superpowers/plans/2026-09-01-richer-notification-content.md - the push notification's "Goals: ..." line now shows P(O2.5) and expected total goals for the most confident new prediction (largest distance from 50%), instead of a bare team-name list
  - Why: closes the "how we got to those analysis" half of R104's discussion - the automatic-scheduling half was already covered by the existing live_poll.yml cron, so this was the one genuinely missing piece
  - Fix: two real fix-and-re-review rounds on Task 1, both caught by code review not the plan itself - (1) the new `build_goals_line()` call sat outside `main()`'s only try/except, so a malformed row would crash the whole notify step with a raw traceback instead of the file's own established non-fatal failure-logging pattern; fixed by wrapping the call and falling back to the original team-name listing. (2) that fallback's `except (ValueError, KeyError)` still wasn't exhaustive - a ragged/short CSV row (a known real scenario per `_new_rows()`'s own docstring) produces a `None` value via `csv.DictReader`'s `restval` default, and `float(None)` raises `TypeError`, not caught by the narrower clause; widened to a bare `except Exception` to match the identical pattern already used at the file's HTTP-POST error handler, plus added a test confirming the underlying function actually raises on a malformed row
  - Blocked by: ~none~
  - Status: done — both tasks implemented, spec-reviewed, and code-quality-reviewed (two fix-and-re-review loops on Task 1). Final whole-round review passed clean (`notify_dashboard_test.py` passes, clean import, no critical issues) - flagged one non-blocking coverage gap (the widened except's TypeError path isn't directly tested, though the underlying fix is correctly reasoned and verified by direct code inspection) which the reviewer explicitly said not to block the push on. Pushed to origin/main. New behavior will be observable the next time live_poll.yml logs a real confirmed-lineup prediction - no separate manual verification needed since main()'s git-diff-reading and HTTP-POST logic are both unchanged, only the string one function builds is different.

- [ ] **R106** — Model accuracy review: calibration fix vs model redesign → *medium*
  - Context: user asked how to improve the model's rate/success rate going forward. Investigated `data/processed/graded_results.csv` (only 6 live-graded matches so far - too small to read anything into) and the real backtest (`data/processed/evals_main.parquet`, 7,305 matches/market since 2022): model Brier score is consistently worse than the market's own implied-odds Brier across every market (goals 0.246 vs 0.239, home/away ~0.21 vs ~0.20, draw 0.184 vs 0.183 - closest gap), confirming the pre-existing "no edge" verdict still holds. Deeper look at over2.5 calibration by decile found the model isn't badly calibrated so much as mis-shaped: it underpredicts in its lowest bucket (32% vs 43% actual, market says 42%) and overpredicts in its highest bucket (71% vs 68% actual, market says 65%) - the model's probability spread is wider/more extreme than reality across every market and every year, while the market's own implied probabilities track actual outcomes more closely in those same tails. Also found the model-vs-market Brier gap has been shrinking every year on every market (e.g. goals: 2022 +0.0083 → 2025 +0.0035; home: +0.0085 → +0.0063), a genuine positive trend already in motion before this conversation
  - Why: distinguishes a real, targeted fix (the tails are the specific place the model diverges from reality) from a vague "make the model better" ask - a post-hoc calibration layer (Platt scaling / isotonic regression fit on `evals_main.parquet`, applied to live predictions before logging) directly targets this specific, confirmed miscalibration without touching the underlying model, and was recommended as the highest-leverage, lowest-risk next step
  - Blocked by: ~none~
  - Status: superseded by **R107** — user said go ahead, but the brainstorming-skill investigation that followed found R106's diagnosis was built on the wrong model.

- [ ] **R107** — Brainstorming reframe: calibrating the wrong model, two real directions forward → *medium*
  - Context: starting the brainstorm for R106's calibration layer, re-explored the codebase and found `evals_main.parquet`/`RESULTS.md` cover the **Dixon-Coles team-strength model** (`footymodel/model.py`, via `footymodel/backtest.py`/`strategy.py`), not the **`LineupModel`** (`footymodel/players.py`) that actually powers the live dashboard (`live/engine.py`). RESULTS.md already contains a far more rigorous version of R106's finding for Dixon-Coles: it's well-calibrated on average but specifically overconfident on the selections where it diverges most from the market (value bets claim +20.8% edge, return -11.7% - confirmed dead end for profit, Phase 3 NO-GO). Separately, RESULTS.md's Phase A found `LineupModel` IS a real, statistically significant accuracy improvement over team-average (t=3.04, replicated full-lineup fix across 5 leagues) - but it still can't beat the closing line in backtest, since bookmakers already price known lineups by kickoff; any real edge could only exist in the live post-lineup-confirmation window before the market re-prices (Phase B - what `live/` is already forward-testing, no backtest guarantee)
  - Why: R106's isotonic/Platt-scaling recommendation was diagnosed against the wrong artifact - proceeding with it as planned would have "fixed" a model that isn't even the one running live, and duplicated analysis RESULTS.md already did more rigorously
  - [x] Option A: sharpen `LineupModel`'s own calibration/accuracy directly (legitimate, has a track record - shots-dispersion fix, full-lineup fix - doesn't require beating the market, just describing reality better)
  - [ ] Option B: test whether the live Phase B window (lineup-confirmation to market re-pricing) has real, measurable edge - the only place profit could plausibly come from per the project's own prior conclusion
  - Blocked by: ~none~
  - Status: done — user chose Option A. Continuing the brainstorm scoped to `LineupModel`'s own calibration/accuracy.

- [ ] **R108** — NB-dispersion test scope: single-league gut-check vs full cross-league replication → *small*
  - Context: exploring Option A's brainstorm found a concrete, previously-untested hypothesis - `LineupModel.predict()`'s Over/Under 2.5 probability (`_ou_prob_over25`, players.py:66) still uses a plain Poisson, even though the shots submodel already discovered (and fixed, `SHOTS_DISPERSION = 3.0`) that a point-estimate Poisson is too sharp-tailed for count data. The same NB-dispersion-sweep methodology (`scripts/shots_calibration_test.py`) has never been run on the goals-level Over/Under prediction
  - Why: the project's own bar for trusting a model change is cross-league replication, not one league - the full-lineup fix wasn't trusted until it held across all 5 big-5 leagues (t=3.04 pooled), whereas the earlier half-lineup version looked good on PL alone and didn't replicate elsewhere (Ligue 1: zero effect)
  - [ ] Quick single-league (E0) gut-check first, decide on full replication after seeing if it looks promising
  - [x] Go straight to the full cross-league replication test, matching the project's established validation standard
  - Blocked by: ~none~
...
- [x] **R109** — Approach for testing the goals-level NB-dispersion hypothesis → *small*
  ...
  - [ ] Rejected: wire a `dispersion` param straight into `LineupModel.predict()` now and sweep via the existing `lineup_test.py` - premature, bakes an unproven parameter into the production model's API before there's evidence it helps
  - [ ] Rejected: put the sweep logic inside `players.py` itself - breaks this codebase's established boundary between the model file and sweep/test scripts (the exact reason `lineup_test.py`/`shots_calibration_test.py` exist as separate files)
  - Blocked by: ~none~
...
- [ ] **R114** — Path forward while the API-Football account is suspended → *small*
  ...
  - [x] Hedge path: add a genuinely independent third data source as a longer-term resilience measure - bigger scope, would need its own proper brainstorm/spec rather than being bolted on ad hoc
...
- [ ] **R120** — `sian.agency/football-api-scraper` test results: strong on lineups, two real caveats found → *small*
  - Context: fetched actor schema/README, then ran 3 small live test calls (~$0.01 total, `maxTotalChargeUsd: 1` cap on each) - `leagueFixtures` (tournamentId=17/EPL, seasonId=76986), `matchLineups` and `matchOdds` for a real finished match (Fulham 0-1 Bournemouth, matchId 14023948)
  - Findings:
    - Fixtures: clean, real EPL matches confirmed (Fulham v Bournemouth, Brighton v Wolves, etc., real scores/venues)
    - Lineups: genuinely the BEST signal of any source tested so far - an explicit `lineupConfirmed: true/false` boolean field (unlike ESPN's ambiguous `starter` flag or RapidAPI's pure timing-heuristic), plus formation, per-player rating, real player names all confirmed correct for the actual match (Bernd Leno, Timothy Castagne, etc. - real Fulham players)
    - Odds: 17 markets returned including "Match goals" (8 rows = 8 different O/U lines) - BUT no field labels which numeric total-goals line each row corresponds to (no "line"/"handicap" field on this market type, unlike Asian Handicap's rows which DO name the handicap). The specific Over/Under 2.5 row can only be inferred by array position/odds-magnitude ordering (assumed ascending 0.5->7.5, third row ~= 2.5) - a real, unconfirmed reliability risk, not a clean labeled field like the other three engines all have
    - **Important discovery**: the field shape (`uniqueTournament`, `ratingVersions`, `statisticsType`, slug-based IDs) and matchId numbering (~14 million range) strongly indicate this actor is itself SofaScore-sourced data, resold via Apify's infrastructure - NOT a genuinely independent underlying provider from the already-existing (currently-403ing) `sofascore_engine.py` fallback. It may still be useful as a more resilient ACCESS PATH to the same source (if Apify's infrastructure isn't currently blocked where direct scraping is), but it does not count as a true "third independent data source" in the sense originally sought
...
- [ ] **R122** — Phase B live-edge test: expectation-setting on timeline + closing-odds gap → *medium*
  ...
  - Blocked by: ~none~
  - Status: done — user chose to build the closing-odds-capture infra now and let it accumulate.
...
- [x] **R125** — Task 1 (closing-odds + CLV) shipped, with 2 review-catches + 1 self-caught regression → *medium*
  - Context: subagent-driven execution of Task 1 from docs/superpowers/plans/2026-09-02-live-closing-odds-clv.md. Spec review passed clean first try (independently re-verified, including the implementer's own self-reported test-stub deviation). First code-quality review found 1 critical bug (test was silently overwriting the REAL rapidapi_budget.json production file) + 2 important robustness gaps (budget spend lost on partial failure; one bad row could lose the whole graded batch) - all three fixed in one targeted commit and re-verified independently (byte-diff + mtime check confirming the real budget file was genuinely untouched, not just coincidentally matching)
  - Why: this is exactly the kind of subtle, easy-to-miss bug (a "pure/data-free" test that wasn't) the two-stage review process exists to catch before it ships
  - Fix: moved `_save_budget()` out of `_fetch_closing_odds()` into `main()`'s `finally` block (unconditional, once per run) - fixed both the test-corruption bug AND the partial-failure-loses-spend gap with one change; added a per-row try/except in `main()`'s loop matching `rapidapi_engine.py`'s own established isolation pattern
  - **Self-caught after both reviews signed off**: re-running grade_results.py's OTHER existing test (`grade_results_by_id_test.py`, pre-existing, CI-wired, unrelated to this task) showed it was now FAILING - `grade_row()`'s signature change (bare `client` -> `clients` dict) broke its two call sites, which still passed a bare mock. Neither the spec reviewer nor either code-quality review pass had run the FULL existing test suite, only the NEW test - a real process gap. Fixed by updating the two call sites to wrap the mock in `{"apifootball": mock_client}`; verified all 4 grade_results-related test scripts (by_id, datetime, columns, clv) pass together after the fix. Widened `_fetch_closing_odds`'s except clause to bare `Exception` (per a second reviewer's minor finding, matching its own docstring's "ANY failure" claim) in the same pass
  - Blocked by: ~none~
  - Status: done. **Process lesson for future tasks**: when a shared/reused function's signature changes, always re-run the FULL existing test suite for that file (not just the new task's test) before considering a task complete - a passing new test does not prove old call sites still work.

- [x] **R126** — live-closing-odds-clv: final whole-round review, ready to finish → *small*
  - Context: independent review across all 5 code commits combined (feature + 3 fixes + CI wiring) - re-ran all 4 grade_results-related tests together, byte-diffed the real rapidapi_budget.json before/after (unchanged mtime), diffed the graded_results.csv migration row-by-row (zero differences in the original 15 columns), AST-checked for unused imports, grepped every other client-construction site in the codebase to confirm the new `clients` dict pattern doesn't conflict with established conventions elsewhere
