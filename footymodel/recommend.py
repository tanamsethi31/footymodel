...
def recommend_for_fixtures(model: DixonColes, fixtures: pd.DataFrame,
                           strategy: str = "confidence", edge: float = DEFAULT_EDGE,
                           min_conf: float = 0.62, market_blend: float = DEFAULT_MARKET_BLEND,
                           bankroll: float = 100.0, kelly_mult: float = 0.25,
                           max_fraction: float = 0.02) -> pd.DataFrame:
    """Generate O/U bets for `fixtures` (needs home_team, away_team,
    odds_over25, odds_under25 — supply the BEST price you can find).

    strategy="confidence" (default, backtest ~break-even): bet the model's
        favored O/U side when its probability >= min_conf. Avoids the model's
        market-disagreement blind spots.
    strategy="value" (backtest ~-2.6%): bet when EV = p*odds-1 > edge. Selects
        for disagreement with the market — historically worse.
    """
    recs = []
    for r in fixtures.itertuples(index=False):
        ...
        if strategy == "confidence":
            # Bet only the single favored side, if confident enough.
            fav = "over25" if mp["p_over25"] >= mp["p_under25"] else "under25"
            candidates = [(fav, {"over25": "p_over25", "under25": "p_under25"}[fav],
                           {"over25": "odds_over25", "under25": "odds_under25"}[fav])]
        else:
            candidates = OU_MARKETS

        for market, pkey, ocol in candidates:
            odds = getattr(r, ocol)
            p_model = mp[pkey]
            p_used = market_blend * p_model + (1 - market_blend) * fair_map[market]
            if strategy == "confidence":
                take = p_model >= min_conf
            else:
                take = (p_used * odds - 1.0) > edge
            if take:
                if strategy == "confidence":
                    stake = round(bankroll * 0.01, 2)  # flat 1% (matches backtest)
                else:
                ...
                recs.append({
                    ...
                    "model_p": round(p_model, 3), "fair_p": round(fair_map[market], 3),
                    "edge_ev": round(p_used * odds - 1.0, 3),
...
def _banner():
    print("!" * 72)
    print("PAPER-TRADE MODE. Best-price backtest: confidence ~break-even, value ~-2.6%.")
...
def demo(league: str, strategy: str, edge: float, min_conf: float,
         market_blend: float, bankroll: float) -> None:
    ...
    slip = recommend_for_fixtures(model, upcoming, strategy=strategy, edge=edge,
                                  min_conf=min_conf, market_blend=market_blend,
                                  bankroll=bankroll)
    if slip.empty:
        print("No bets found for this matchday at the current thresholds.")
        return
...
def main():
    ...
    p.add_argument("--strategy", choices=["confidence", "value"], default="confidence",
                   help="confidence (default, ~break-even) or value (~-2.6%)")
    p.add_argument("--edge", type=float, default=DEFAULT_EDGE, help="value mode: min EV")
    p.add_argument("--min-conf", type=float, default=0.62, help="confidence mode: min model prob")
    ...
    if args.fixtures:
        ...
        _banner()
        slip = recommend_for_fixtures(model, fixtures, strategy=args.strategy,
                                      edge=args.edge, min_conf=args.min_conf,
                                      market_blend=args.market_blend, bankroll=args.bankroll)
        print(slip.to_string(index=False) if not slip.empty else "No bets found.")
        log_paper_trades(slip)
    else:
        demo(args.league, args.strategy, args.edge, args.min_conf,
             args.market_blend, args.bankroll)
