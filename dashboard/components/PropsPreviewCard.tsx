...
export default function PropsPreviewCard({
  ...
  const [open, setOpen] = useState(emphasis === "today" && (live || hasPlayers));
  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between gap-2 flex-wrap">
        <span className={`font-medium flex items-center gap-2 ${titleEmphasisClass(emphasis)}`}>
          <Chevron open={open} />
      ...
      <div className="mt-3 flex items-center gap-2">
        <p className={`text-xs ${bodyEmphasisClass(emphasis)}`}>
          {hasPlayers
